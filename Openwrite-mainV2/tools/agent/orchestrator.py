...
class OpenWriteOrchestrator:
    ...
    def _handle_outline_confirmation(self) -> OrchestratorResult:
        ...
        if not self.story_planning_store.promote_outline(confirmed=True):
            ...
            self.state.blocking_reason = (
                "outline_revision_conflict" if has_draft else "missing_outline_draft"
            )
            self.state.last_agent_action = (
                "blocked_outline_promotion_revision_conflict"
                if has_draft
                else "blocked_outline_promotion_missing_draft"
            )
            self.state_store.save(self.state)
            return OrchestratorResult(
                message=(
                    "大纲在暂存后发生变化，已拒绝覆盖。请重新读取目标片段并生成补丁。"
                    if has_draft
                    else "大纲草稿缺失。请先生成或暂存大纲修改。"
                ),
                stage=self.state.stage,
                blocked=True,
                next_action="read_outline" if has_draft else "prepare_outline_document",
            )

        self._sync_runtime_caches(sync_outline=True, sync_characters=False)
        advanced_to_preflight = self.state.stage in {
            BookStage.DISCOVERY,
            BookStage.FOUNDATION,
            BookStage.ROLLING_OUTLINE,
            BookStage.REVIEW_AND_REVISE,
        ...
        message = (
            "大纲范围已确认。下一步进入章节预检。"
            if advanced_to_preflight
            else f"大纲增量修改已确认，当前流程保持在 {self.state.stage.value}。"
        )
        return OrchestratorResult(
            message=message,
            stage=self.state.stage,
    ...
    def _handle_outline_generation(self, text: str) -> OrchestratorResult:
        ...
        if (
            ...
            return self._stage_blocked_result(
                "已存在确认版大纲。为避免整篇漂移，请先读取目标片段，再使用增量补丁修改。",
    ...
    def _build_style_documents(
        ...
        craft_root = resolve_craft_dir(self.project_root)
        for name in (
            "dialogue_craft.md",
            "scene_craft.md",
            "rhythm_craft.md",
