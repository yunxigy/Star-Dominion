...
class TruthFilesManager:
    ...
    def __init__(self, project_root: Path, novel_id: str):
        ...
        self.state_history_dir = self.world_dir / "state_history"
    ...
    def load_truth_files_at_chapter(self, chapter_number: int | str) -> TruthFiles:
        """Load the latest truth snapshot at or before a chapter boundary."""

        try:
            target = max(0, int(chapter_number))
        except (TypeError, ValueError):
            return self.load_truth_files()
        ...
        if self.snapshots_dir.exists():
            for path in self.snapshots_dir.glob("snapshot_*.json"):
                try:
                    ...
                    number = int(snapshot.get("chapter_number", -1))
                    if number <= target:
                        candidates.append(
                            (number, str(snapshot.get("created_at") or ""), path)
                        )
                except (OSError, json.JSONDecodeError, TypeError, ValueError) as exc:
                    logger.warning("Failed to inspect snapshot %s: %s", path, exc)

        if not candidates:
            truth = self.load_truth_files()
            truth.metadata["_source"] = {
                "kind": "current_runtime_fallback",
        ...
        try:
            ...
            if not isinstance(files, dict):
                raise ValueError("快照 files 不是对象")
            truth = TruthFiles(
            ...
            truth.metadata = {
                attr_name: self._default_metadata(
                    attr_name, getattr(truth, attr_name)
                )
                for attr_name in ("current_state", "ledger", "relationships")
            }
            truth.metadata["_source"] = {
                "kind": "snapshot",
                "chapter_number": int(snapshot.get("chapter_number", -1)),
                "snapshot_id": str(snapshot.get("id") or selected.stem),
            }
            return truth
        except (OSError, json.JSONDecodeError, KeyError, TypeError, ValueError) as exc:
            logger.warning("Failed to load historical truth snapshot %s: %s", selected, exc)
            truth = self.load_truth_files()
            truth.metadata["_source"] = {
                "kind": "current_runtime_fallback",
                "chapter_number": target,
    ...
    def record_chapter_update(
        ...
        """Persist the accepted state delta for later historical context assembly."""
        ...
        target = self.state_history_dir / f"{chapter_id}.json"
        payload = {
    ...
    def load_recent_chapter_updates(
        self,
        limit: int = 5,
        *,
        before_chapter: int | str | None = None,
    ) -> list[dict[str, Any]]:
        """Load accepted state deltas, optionally bounded by a chapter."""

        if not self.state_history_dir.exists():
            return []
        boundary: int | None = None
        if before_chapter is not None:
            try:
                boundary = int(before_chapter)
            except (TypeError, ValueError):
                boundary = None

        records: list[tuple[int, dict[str, Any]]] = []
        for path in self.state_history_dir.glob("ch_*.json"):
            match = re.search(r"(\d+)", path.stem)
            number = int(match.group(1)) if match else -1
            if boundary is not None and number > boundary:
                continue
            try:
                payload = json.loads(path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError) as exc:
                logger.warning("Failed to load chapter state history %s: %s", path, exc)
                continue
            if isinstance(payload, dict):
                records.append((number, payload))
        records.sort(key=lambda item: item[0])
        return [payload for _, payload in records[-max(1, int(limit)) :]]
