...
class NovelApplicationService:
    ...
    def initialize(
        ...
        *,
        ...
        try:
            # The initializer is also a CLI command and prints Unicode progress
            # markers. Studio may run with a Windows GBK stdout, so keep CLI
            # progress out of the HTTP process instead of letting it fail the
            # actual filesystem initialization.
    ...
    def write_chapter(self, payload: dict[str, Any]) -> dict[str, Any]:
        chapter_id, args = self._prepare_write_args(payload)
    ...
    def write_and_review_chapter(self, payload: dict[str, Any]) -> dict[str, Any]:
        """Generate and review a candidate before committing it."""

        chapter_id, args = self._prepare_write_args(payload)
        executor = self._writer_executor or self._default_write_and_review_executor
        if not self._task_lock.acquire(blocking=False):
            raise NovelServiceError("已有写作或审稿任务正在运行", code="PROJECT_BUSY")
        try:
            result = executor(self.project_root, args)
            ...
            self._task_lock.release()
        self._ensure_ok(result, fallback="写作审查失败")
        if self._writer_executor is not None:
            self._record_write_lifecycle(chapter_id, result)
        return result
    ...
    @staticmethod
    def _default_write_and_review_executor(
        root: Path,
        args: dict[str, Any],
    ) -> dict[str, Any]:
        from tools.chapter_pipeline import execute_write_and_review_chapter

        result: dict[str, Any] = execute_write_and_review_chapter(root, args)
        return result
