...
def test_service_initialization_does_not_write_unicode_progress_to_gbk_stdout(
    tmp_path: Path,
):
    output = io.BytesIO()
    ...
    with redirect_stdout(stream):
        service = NovelApplicationService.initialize(tmp_path, "gbk_demo", "编码测试")
    stream.flush()

    assert service.novel_id == "gbk_demo"
    assert (tmp_path / "novel_config.yaml").is_file()
...
def test_service_continuity_and_error_contract(tmp_path: Path):
    ...
    assert created["continuity"]["relationship_graph"] == {
        "nodes": [],
        "edges": [],
