...
- Studio 左侧“大纲”从 `src/outline.md` 实时投影卷/幕/节/章树；原文仍可直接编辑，“故事”不混入大纲。
- Studio 大纲树支持就地改名、新增卷/幕/节/章、删除未含正文的节点；删除后，后续同类节点自动连续补位，确认框先展示级联影响。每次只增量写回对应 Markdown 片段，并自动建立 Git 存档。
- 搜索可单独限定“大纲”，对应 `src/outline.md`。
- `src/outline.md` 是大纲唯一语义真源，`data/hierarchy.yaml` 仅为派生缓存。
- Studio 关系拓扑与 ReAct 的 `get_world_relations` 共用人物和世界实体文件，不使用 `graph.yaml`。
- 修改一条关系时先用 `edit_world_relation(confirm=false)` 预览 diff；只有用户明确确认后才携带同一 `base_revision` 与 `confirm=true` 写入。revision 冲突必须重新预览。

## 结构树与智能选章

- 用 `get_outline_structure` 读取与 Studio 同源的树、原文行号、已有正文状态和下一章建议；该工具只读。
- 用 `edit_outline_structure` 携带最新 revision 执行 `rename`、`add_child`、`add_after` 或 `delete`；`delete` 会删除目标子树，并按全书顺序压缩其后同类卷/幕/节/章的编号，附录不参与。先以 `confirm=false` 预览包含全部补位变化的 diff，用户明确确认后才以相同 revision 和 `confirm=true` 写入。陈旧 revision、重复章节号、包含已有正文的删除，以及会让已有正文 `ch_XXX` 换号的补位必须拒绝。无法识别编号的标题保持不变并报告跳过。
- 将 Markdown 的 H1/H2/H3/H4 投影为卷/幕/节/章；将附录隔离为附录树，不混入正文层级统计。
- 默认推荐大纲顺序中最早尚无正文的章纲，不按最大章节号盲猜。
- 从所选章纲、所属节摘要和最近三章实际字数预填章节 ID、要求和目标字数。
- 写作前携带大纲 `revision`；revision 已变化时重新读取建议，不得写入旧目标。
- 已有正文的章纲只允许打开，不得由“创建建议章节”覆盖。
...
**影响范围**:
- src/outline.md（唯一真源，增量修改）
- data/hierarchy.yaml（随后重建的派生缓存）
...
| 可写大纲 | `data/novels/{id}/src/outline.md` |
