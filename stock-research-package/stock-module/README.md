...
- 宝妈指数保持独立，只作反向情绪温度计，不读取或改变候选评分。目前页面保留独立状态卡，正式采集与历史展示仍是后续阶段。
...
| SQLite | 候选快照、模型密文、任务、报告和模型目录缓存 | `data/hub.db`，不保存路由令牌 |
...
1. 打开工作台，先阅读九点猫研晨报、右侧个人策略和盘后至开盘前重要消息。
...
| `GET` | `/api/v1/candidates` | 获取猫研和用户策略候选 |
...
| `GET` | `/api/v1/stocks/{symbol}/research-context` | 聚合九研与个人策略的独立选股依据；任意合法主板代码均可查询 |
