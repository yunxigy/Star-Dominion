...
const WebpToJpg: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const { files, inputProps, triggerUpload, clearFiles, removeFile } = useFileUpload('image/webp');
  ...
  return (
    <div className="space-y-3">
      <input {...inputProps} />
      {files.length === 0 ? (
        <UploadZone onUpload={triggerUpload} accept="image/webp" label="上传 WebP 图片" sublabel="支持 .webp 格式，透明区域将填充白色" />
      ) : (
        <>
