...
const BmpToJpg: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const { files, inputProps, triggerUpload, clearFiles, removeFile } = useFileUpload('image/bmp,image/x-ms-bmp,image/x-windows-bmp,.bmp');
  ...
  return (
    <div className="space-y-3">
      <input {...inputProps} />
      {files.length === 0 ? (
        <UploadZone onUpload={triggerUpload} accept=".bmp,image/bmp" label="上传 BMP 图片" sublabel="支持 .bmp 位图格式" />
      ) : (
        <>
