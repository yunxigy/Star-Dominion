...
const PngToWebp: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  return (
    <div className="space-y-3">
      <input {...inputProps} />
      {files.length === 0 ? (
        <UploadZone onUpload={triggerUpload} onDropFiles={handleFiles} accept="image/png" label="上传 PNG 图片" sublabel="转换为 WebP 格式，保留透明通道" />
      ) : (
        <>
