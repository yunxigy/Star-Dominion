...
Star Dominion 的统一主站和在线工具目录。当前注册表包含 **212 个工具、19 个分类**，大部分功能在浏览器本地运行；需要服务端处理的工具会在页面中明确显示上传和隐私边界。

## 工具分类

| 分类 | 数量 | 主要能力 |
| --- | ---: | --- |
| PDF 工具 | 20 | 合并、拆分、压缩、页面处理、水印、加密、图文提取和格式转换 |
| 图片工具 | 11 | 压缩、裁剪、缩放、拼接、取色、证件照和 Base64 |
| 图片增强 | 10 | 清晰度、亮度、锐化、马赛克、文字、截图和封面 |
| 格式转换 | 10 | JPG、PNG、WebP、SVG、BMP、HEIC 和 ICO 转换 |
| 开发者工具 | 29 | 数据格式、编码、哈希、API、OpenAPI、网络、日志、Cron、CIDR 和 SQL |
| 计算器 | 16 | 日期、贷款、复利、百分比和常用单位换算 |
| 趣味工具 | 7 | 抽奖、随机数、随机选择和生活随机工具 |
| 测评中心 | 20 | 趣味、人格、倾向与边界三组互动测评，含 MBTI 40 题扩展版 |
| 塔罗星座 | 11 | 塔罗、星座、运势和配对 |
| 鼠标测试 | 10 | CPS、反应速度、DPI、滚轮、回报率和拖拽 |
| 文档处理 | 9 | OCR、翻译、摘要、语法、文本对比、论文查重和文档转换中心 |
| 音频处理 | 3 | 音频转换、NCM 处理和变声 |
| 视频工具 | 1 | 抖音与 B 站单个公开视频解析、清晰度选择和临时下载 |
...
| 文本工具 | 9 | 文本清理、提取、替换与文件整理 |
| 站长工具 | 12 | SEO 元数据、网站配置与网络状态检查 |
...
`/tool/video-parser-downloader` 支持解析并下载无需登录即可访问的抖音、哔哩哔哩单个公开视频。浏览器只接收短期解析令牌、清晰度选项和本站任务 ID；原始媒体地址与 yt-dlp 格式选择器不会暴露给前端。用户提交的视频链接会发送到本站 `video-downloader` 后端，由后端访问对应平台并临时处理，因此该工具标记为 `third-party-api`。

服务默认监听 `127.0.0.1:8011`，运行需要 yt-dlp；B 站部分高清源还需要 FFmpeg 合并音视频。当前不支持合集、多 P、会员、付费、私密、直播或批量内容，也不保证去除画面已有水印或提供固定清晰度。解析凭证、任务与完成文件均会自动过期，部署与接口说明见 [`video-downloader/README.md`](../video-downloader/README.md)。
