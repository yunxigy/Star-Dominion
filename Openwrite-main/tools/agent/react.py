...
class ReActAgent:
    ...
    async def run(
        ...
        on_tool_result: Optional[Callable[[str, str, str], None]] = None,
        ...
        for turn in range(self.max_turns):
            ...
            for tool_call in response.tool_calls:
                ...
                if on_tool_call:
                    r = on_tool_call(tc_id, tc_name, tc_args)
                    if inspect.isawaitable(r):
                        await r
                ...
                try:
                    ...
                    if on_tool_result:
                        r = on_tool_result(tc_id, tc_name, result)
                        if inspect.isawaitable(r):
                            await r
                    messages.append(
                        Message(
                    ...
                    if on_tool_result:
                        r = on_tool_result(tc_id, tc_name, error_result)
                        if inspect.isawaitable(r):
                            await r
                    messages.append(
                        Message(
...
OPENWRITE_TOOLS = [
    ToolDefinition(
        name="write_chapter",
        description="写一章草稿。不传 chapter_id 则自动创建下一章（如 ch_001, ch_002...）。传 chapter_id 则写入指定章节。",
        parameters={
            "type": "object",
            "properties": {
                "chapter_id": {"type": "string", "description": "章节 ID（如 ch_005），不传则自动创建下一章"},
...
1. 用户给出指令后，先了解当前状态（调用 get_status 和 list_chapters）
2. 根据需要调用工具
3. 向用户汇报进展
4. 直到任务完成
